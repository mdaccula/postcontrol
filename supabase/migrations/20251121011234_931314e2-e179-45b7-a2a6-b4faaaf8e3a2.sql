-- ============================================================================
-- FASE 1: Refatorar Sistema de Metas para Suportar Múltiplas Regras
-- ============================================================================

-- 1. Adicionar coluna para rastrear qual regra foi atingida
ALTER TABLE user_event_goals 
ADD COLUMN IF NOT EXISTS achieved_requirement_id UUID REFERENCES event_requirements(id);

-- 2. Criar índice para performance
CREATE INDEX IF NOT EXISTS idx_user_event_goals_requirement 
ON user_event_goals(achieved_requirement_id);

-- 3. Dropar função antiga para recriar com novo retorno
DROP FUNCTION IF EXISTS public.check_and_update_user_goal(uuid, uuid);

-- 4. Refatorar função check_and_update_user_goal para suportar múltiplas regras
CREATE FUNCTION public.check_and_update_user_goal(p_user_id uuid, p_event_id uuid)
RETURNS TABLE(
...
BEGIN
  ...
  FOR v_requirement IN
    SELECT id, required_posts, required_sales
