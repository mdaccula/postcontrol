-- Dropar função antiga para recriar com novo retorno
DROP FUNCTION IF EXISTS public.get_top_promoters_ranking(uuid, integer);

-- Recriar função get_top_promoters_ranking para incluir achieved_requirement_id
CREATE FUNCTION public.get_top_promoters_ranking(p_event_id uuid, p_limit integer DEFAULT 10)
RETURNS TABLE(
  user_id uuid,
  full_name text,
  avatar_url text,
