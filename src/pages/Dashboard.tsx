...
import { ArrowLeft, TrendingUp, Award, Calendar, LogOut, MessageCircle, Building2, ChevronDown, Camera, User, Lock, Send, Users,
...
const BadgeDisplay = lazy(() => import("@/components/BadgeDisplay").then(m => ({
  default: m.BadgeDisplay
})));
const AIInsights = lazy(() => import("@/components/AIInsights").then(m => ({
...
const Dashboard = () => {
  ...
  // React Query hooks
  const {
    data: userAgenciesData,
    isLoading: isLoadingAgencies
  } = useUserAgenciesQuery(user?.id);
  const {
    data: adminSettingsData,
    isLoading: isLoadingSettings
  } = useAdminSettingsQuery(["ai_insights_enabled", "badges_enabled", "whatsapp_number"]);
  ...
  const filteredSubmissions = useMemo(() => {
    ...
    return submissions.filter(s => {
      const eventAgencyId = s.posts?.events && typeof s.posts.events === 'object' ? (s.posts.events as any).agency_id : null;
  ...
  const filteredSubmissionsByEvent = useMemo(() => {
    ...
    return selectedHistoryEvent === "all" ? filteredSubmissions : filteredSubmissions.filter(s => s.posts?.event_id === selectedHistoryEvent);
  ...
  useEffect(() => {
    if (userAgenciesData && userAgenciesData.length > 0) {
      ...
      setSelectedAgencyId(validSavedAgency ? savedAgencyId : userAgenciesData[0].id);
    }
  }, [userAgenciesData]);
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async (newData: Partial<typeof profile>) => {
      const {
        error
      } = await sb.from("profiles").update(newData).eq("id", user!.id);
  ...
  const saveAvatar = async () => {
    ...
    try {
      ...
      const {
        data: oldFiles
      } = await sb.storage.from("screenshots").list("avatars", {
        search: user.id
      });
      ...
      const {
        error: uploadError
      } = await sb.storage.from("screenshots").upload(fileName, avatarFile, {
        upsert: true
      });
      ...
      const {
        data: signedData,
        error: signedError
      } = await sb.storage.from("screenshots").createSignedUrl(fileName, 31536000);
  ...
  if (loading) {
    return <div className="min-h-screen bg-gradient-to-br from-background via-muted to-background p-8">
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-muted to-background">
      <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 sm:gap-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 w-full overflow-hidden">
                  <Avatar className="h-16 w-16 sm:h-20 sm:w-20 md:h-24 md:w-24 ring-4 ring-primary/20 shadow-lg flex-shrink-0">
                   ...
                   <div className="space-y-2 min-w-0 flex-1">
                     <div className="flex items-center gap-2 flex-wrap">
                       ...
                       {isMasterAdmin && <Badge variant="default" className="bg-purple-500">
                           Master Admin
                         </Badge>}
                       {isAgencyAdmin && !isMasterAdmin && <Badge variant="default" className="bg-blue-500">
                           Agency Admin
                         </Badge>}
                     </div>
                     ...
                     {userAgenciesData && userAgenciesData.length > 1 && <Select value={selectedAgencyId} onValueChange={setSelectedAgencyId}>
                         ...
                         <SelectContent>
                           {userAgenciesData.map((agency: any) => <SelectItem key={agency.id} value={agency.id}>
                               {agency.name}
                             </SelectItem>)}
                         </SelectContent>
                       </Select>}
                 ...
                 <div className="flex flex-wrap items-center gap-3">
                   ...
                   <Button onClick={() => navigate("/submit")} className="bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70">
                     Enviar Nova Postagem
                   </Button>

                   {/* ✅ ITEM 7: Botão para Guest Dashboard */}
                   {isGuest && guestData && <Button onClick={() => navigate("/guest-dashboard")} variant="outline" className="gap-2">
                       <Users className="h-4 w-4" />
                       Painel Convidado
                     </Button>}

                   {isMasterAdmin && <Button onClick={() => navigate("/master-admin")} variant="outline">
                      Master Admin
                    </Button>}

                  {isAgencyAdmin && <Button onClick={() => navigate("/admin")} variant="outline">
                      Painel Admin
                    </Button>}
        ...
        {/* Badges */}
        {badgesEnabled && <Suspense fallback={<Skeleton className="h-32 w-full" />}>
            <BadgeDisplay />
          </Suspense>}

        {/* Botão Convidar Amiga para Divulgação */}
        <motion.div initial={{
        opacity: 0,
        y: 20
      }} animate={{
        ...
        delay: 0.2
          ...
          <Card className="p-6 bg-gradient-to-r from-[#25D366]/10 to-[#128C7E]/10 border-[#25D366]/20">
            <div className="flex-col flex items-center justify-between px-[20px] py-[10px]">
              ...
              <Button onClick={() => setInviteDialogOpen(true)} className="bg-gradient-to-r from-[#25D366] to-[#128C7E] hover:from-[#128C7E] hover:to-[#25D366] text-white text-center text-lg">
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Select value={selectedInviteEvent} onValueChange={setSelectedInviteEvent}>
                  <SelectTrigger id="invite-event">
                    <SelectValue placeholder="Selecione um evento ativo..." />
                  </SelectTrigger>
                  <SelectContent>
                    {filteredEvents?.filter(e => e.is_active).length > 0 ? filteredEvents.filter(e => e.is_active).map(event => <SelectItem key={event.id} value={event.id}>
                          {event.title}
                        </SelectItem>) : <div className="px-2 py-6 text-center text-sm text-muted-foreground">
              ...
              const selectedEvent = filteredEvents.find(e => e.id === selectedInviteEvent);
              if (!selectedEvent) return;
              try {
                // Buscar template de mensagem da agência
                const {
                  data: agencyData
                } = await supabase.from('agencies').select('invite_message_template').eq('id', selectedAgencyId).single();
                const messageTemplate = agencyData?.invite_message_template || 'Oi, queria te indicar para ser uma divulgadora da {agencyName}, participar do evento {eventTitle} e ter sua cortesia batendo os requisitos das postagens, chamar o http://bit.ly/Contato_MD para ele te incluir no grupo de Promo';

                // Substituir placeholders
                const message = messageTemplate.replace('{agencyName}', agencyName).replace('{eventTitle}', selectedEvent.title);
            ...
            }} disabled={!selectedInviteEvent} className="w-full bg-gradient-to-r from-[#25D366] to-[#128C7E] hover:from-[#128C7E] hover:to-[#25D366] text-white">
                Abrir WhatsApp
              </Button>
            </div>
          </DialogContent>
        ...
        <DashboardStats approvedCount={filteredSubmissions.filter(s => s.status === 'approved').length} totalSubmissions={filteredSubmissions.length} activeEventsCount={filteredEventStats.length} lastSubmissionDate={lastSubmission?.submitted_at || null} eventStats={filteredEventStats} />
        ...
        <Tabs defaultValue="statistics" className="space-y-6">
          ...
          <TabsContent value="statistics" className="space-y-6">
            <Card className="p-6">
              ...
              <div className="space-y-6">
                {filteredEventStats.length > 0 ? filteredEventStats.map(stat => {
                // ✅ Verificar se evento é de análise manual (sem metas definidas)
                const isManualReview = !stat.totalRequired || stat.totalRequired === 0;
                return <div key={stat.eventId} className="space-y-3">
                        <div className="space-y-3 p-4 rounded-lg bg-muted/30">
                          <div className="flex items-center justify-between">
                            <div className="space-y-1 flex-1">
                              <div className="flex items-center gap-2">
                                <h3 className="font-semibold text-lg">{stat.eventTitle}</h3>
                                {isManualReview && <Badge variant="outline" className="border-purple-500 text-purple-700 dark:text-purple-400 text-xs">
                                    <ClipboardCheck className="w-3 h-3 mr-1" />
                                    Análise Manual
                                  </Badge>}
                              </div>
                              {!isManualReview ? <p className="text-sm text-muted-foreground">
                                  {stat.submitted} de {stat.isApproximate ? "~" : ""}
                                  {stat.totalRequired} posts aprovados
                                </p> : <p className="text-xs text-purple-600 dark:text-purple-400">
                                  Este evento não possui metas automáticas
                                </p>}
                            </div>
                            {!isManualReview && <Badge variant={stat.percentage >= 100 ? "default" : "secondary"} className="text-lg px-4 py-2">
                      ...
                      {/* Goal Progress Badge */}
                      {user && !isManualReview && <GoalProgressBadge eventId={stat.eventId} userId={user.id} variant="detailed" />}
                    </div>;
              }) : <p className="text-center text-muted-foreground py-8">Nenhum evento ativo no momento</p>}
                {filteredEventStats.length === 0 && eventStats.length > 0 && <p className="text-center text-muted-foreground py-4 text-sm">Sem eventos nesta agência</p>}
              </div>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="history" className="space-y-6">
            <DashboardSubmissionHistory submissions={filteredSubmissionsByEvent} events={filteredEvents} selectedEvent={selectedHistoryEvent} onEventChange={setSelectedHistoryEvent} onDeleteSubmission={setSubmissionToDelete} SubmissionImageDisplay={SubmissionImageDisplay} />
          </TabsContent>

          {/* Profile Tab */}
          <TabsContent value="cadastro" className="space-y-6">
            <DashboardProfile profile={profile} avatarPreview={avatarPreview} avatarFile={avatarFile} uploading={uploading} uploadProgress={uploadProgress} instagram={instagram} selectedGender={selectedGender} newPassword={newPassword} confirmPassword={confirmPassword} isAgencyAdmin={isAgencyAdmin} user={user} onAvatarChange={handleAvatarChange} onSaveAvatar={saveAvatar} onInstagramChange={setInstagram} onSaveInstagram={async () => {
            if (!user) return;
            try {
              const {
                error
          ...
          }} onGenderChange={setSelectedGender} onSaveGender={saveGender} onNewPasswordChange={setNewPassword} onConfirmPasswordChange={setConfirmPassword} onChangePassword={changePassword} onFollowersRangeChange={async value => {
            await updateProfileMutation.mutateAsync({
              followers_range: value
            });
